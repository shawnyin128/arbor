...
For active engineering planning continuations that route to `brainstorm`, the downstream visible answer must be the standard brainstorm checkpoint, not an ad hoc planning summary. For English prompts, the final rendered response must use the brainstorm headings exactly: `Understanding And Recommendation`, `How I Would Handle This`, `Suggested Small Steps`, `How I Would Validate Each Step`, `Default Decisions I Made`, `Expected Delivery`, and `Next`. For non-English prompts, it must use localized heading equivalents in the user's active chat language, in the same order and with the same table sections. Do not use substitutes such as `Brainstorm Checkpoint`, `Recommended plan`, or prose-only acceptance criteria.

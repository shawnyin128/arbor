...
- a real Codex or Claude Code process: use `codex exec` for non-hook workflow
  replay, but use a trusted interactive Codex or desktop session when proving
  Codex project hook firing because `codex exec` is not a reliable project-hook
  firing proof;
- for Claude, use `claude -p --plugin-dir` when the case does not require an
...
| R12 | Finalization/public release actions were not cleanly gated. | Natural "publish/release" intent through `$converge`, not public `$release`, with and without explicit authorization. | `converge -> internal release(finalize_feature)` | Preparation can occur, but finalization commit, push, PR, tag, and publish require explicit user authorization; unauthorized cases stop with readable confirmation need. |
| R13 | "Publish" should finish the release path after validation passes. | Natural "publish" intent after converged feature and explicit permission, held under `$converge` until internal release finalization. | `converge -> internal release(finalize_feature)` | Commit/push/publish action that was explicitly authorized succeeds after convergence; response includes action metadata; no unrelated work is included. |
| R14 | Local Codex and Claude plugin caches, marketplace snapshots, or installed records diverged from source, or cache sync pruned old versions still referenced by running hook snapshots. | Post-change cache verification. | Runtime cache sync check | Harness diffs shipped runtime files from the source plugin against `~/.codex/plugins/cache/...` and `~/.claude/plugins/cache/...`, checks Codex's local marketplace snapshot when present, checks Claude's `installed_plugins.json` record, and verifies sync policy preserves older version cache directories while replacing only the target version and refreshing stale hook adapter files in older caches; runtime smoke uses cached copy, not only worktree copy. |
| R15 | Startup/session-start context was not read before a direct overview. | Fresh Codex prompt asks what the project does and explicitly says to load startup context before answering directly. | direct answer after startup context | Response is source-grounded from `AGENTS.md`, recent git log, `.arbor/memory.md`, and `git status`; it is not treated as `$arbor` project-summary output. Claude hook output includes all sections under budget. |
| R16 | Codex startup relied on inert hook intent or untrusted hook config instead of reliable `AGENTS.md` bootstrap. | Fresh Codex run in initialized temp repo. | `arbor` startup protocol | Agent explicitly loads or reproduces startup context order; `.codex/hooks.json` alone is not treated as proof of injected context, and initialized Codex hooks use executable `SessionStart` schema plus `.codex/hooks/` wrappers. |
| R17 | `.arbor/memory.md` was not updated during uncommitted Arbor work. | Stop after Arbor-managed edit before commit. | Any active workflow skill plus memory guard | `.arbor/memory.md` exists and names active feature, changed paths, checkpoint, risks, and next step before assistant stops. Codex Stop-hook proof must come from a trusted interactive Codex or desktop session, not `codex exec` alone. |
| R18 | Resolved memory remained after commit/publish. | Commit after previously dirty Arbor workflow state. | `arbor` memory guard or internal release checkpoint | Resolved in-flight memory is pruned after successful commit; git history becomes authoritative. |
| R19 | Memory hygiene hook trigger rate was under-tested. | Case corpus spanning stops, handoffs, failed checks, release gates, cache syncs, direct answers, read-only turns, and no-write turns. | Hook intent review plus real dirty-state prompts | Positive cases emit memory-hygiene packet or result in memory update; Codex project registration writes executable `Stop` hook schema plus `.codex/hooks/arbor-stop-memory-hygiene`; negative cases remain quiet. |
| R20 | Hook/session logic was claimed without real adapter output. | Direct execution of shared `hooks/session-start` through Codex `.codex/hooks/` and Claude `.claude/hooks/` wrappers plus trusted interactive Codex or desktop hook replay where available. | SessionStart hook | Hook stdout includes startup packet for `startup` and `resume`, stays empty for `clear`, and project initialization writes the current runtime's project hook surface under `.codex/` or `.claude/`. `codex exec` may validate ordinary workflow replay, but it is not sufficient proof that Codex project hooks fired. |
| R21 | Structured output and rendered UI were not checked across public entrypoints. | Explicit invocation of public workflow entrypoints plus internal fixture checks for internal stages. | `brainstorm`, `feedback`, `converge`, plus internal develop/evaluate/release fixtures | Internal raw packet is not the primary final answer; visible text follows each public skill's designed sections and status style. |
...
| R23 | Entry from the middle of the workflow could fail. | Direct prompts for converge with equivalent review evidence already present; public develop/evaluate/release prompts route-correct to converge or the appropriate public owner. | Chosen public skill or route correction | Valid mid-chain public entries work; incomplete entries block with readable missing evidence instead of fabricating state. |
...
| R25 | `$arbor` behaved like a generic project-summary, subjective health-advice, or maintenance report. | Paired Codex and Claude `$arbor` runs ask for a detect-only Arbor framework check covering only Arbor-created files and hook surfaces. | `arbor` minimal framework check | Output uses exactly one Surface, Required, Status, Evidence, Repair table plus one Result line; project-level hooks are required for the selected runtime; output does not render process-state, docs/review, feature-registry, project-guide drift, memory-progress, migration, health, maintenance, or project-status sections. |
...
| R35 | Simulation collapse feedback was treated as a direct schema-first repair instead of diagnosing system dynamics. | Feedback or reviewer note says a live artifact is dominated by one candidate class, tests pass, or a simulation/agent loop collapses. | `feedback -> brainstorm` with closed-loop diagnostics | The route requires state trajectory, event/effect trace, candidate source or candidate pool trace, decision distribution, dimension checks, budget balance, raw trace, live artifact quality, and weak-pass gap evidence before converge or release can accept the fix. |
...
- R35 on Codex for closed-loop diagnostics, simulation collapse, or schema-first repair-routing changes;

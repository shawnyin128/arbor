...
| R25 | `$arbor` behaved like a generic project-summary command. | Paired Codex and Claude `$arbor` runs ask for Arbor context, memory, and hook health without a product overview. | `arbor` context/status check | Output names Arbor context, memory, and hook health; it does not render a product summary, recommend product next actions, or infer release work from memory prose. |

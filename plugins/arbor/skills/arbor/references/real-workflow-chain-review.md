...
| R14 | Local Codex and Claude plugin caches, marketplace snapshots, or installed records diverged from source. | Post-change cache verification. | Runtime cache sync check | Harness diffs shipped runtime files from the source plugin against `~/.codex/plugins/cache/...` and `~/.claude/plugins/cache/...`, checks Codex's local marketplace snapshot when present, and checks Claude's `installed_plugins.json` record; runtime smoke uses cached copy, not only worktree copy. |

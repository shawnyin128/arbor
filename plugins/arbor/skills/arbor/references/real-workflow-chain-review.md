...
| R25 | `$arbor` behaved like a generic project-summary or subjective health-advice command. | Paired Codex and Claude `$arbor` runs ask for a detect-only Arbor framework check covering context, memory, and hook surfaces. | `arbor` deterministic framework check | Output uses fixed rows with Category, Check, Status, Evidence, Fixability, and Repair action; hook surfaces are separated; output does not render product summaries, Healthy/Maintenance/Suggested-next-action sections, or release/feature advice inferred from memory prose. |

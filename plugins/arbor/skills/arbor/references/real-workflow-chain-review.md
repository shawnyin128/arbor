...
| R20 | Hook/session logic was claimed without real adapter output. | Direct execution of Claude `hooks/session-start` plus project-level `.claude/settings.json` registration replay where available. | SessionStart hook | Hook stdout includes startup packet for `startup` and `resume`, stays empty for `clear`, and project initialization writes the Claude project hook surface under `.claude/`. |

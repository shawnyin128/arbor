...
def render_memory_hygiene_packet(sections: list[ContextSection]) -> str:
    lines = [
        ...
        "## Agent Instructions",
        "",
        "- Decide whether `.arbor/memory.md` would help the next agent resume current Arbor work that is not committed or durable elsewhere.",
        "- If an update is needed, edit only the project-local `.arbor/memory.md`.",
        "- Write or refresh memory when a fresh agent would otherwise lose Arbor-relevant discussion, checkpoint files, unresolved decisions, or the first resume action.",
        "- Do not add direct one-off answers, stable project rules, detailed review evidence, release history, cache/scratch notes, or anything already committed or durable elsewhere.",
        "- Remove or shrink resolved memory when git history, durable docs, review files, or `AGENTS.md` already carry the state.",
